...
RUN sed -i "s/^\(shared_preload_libraries.*\)'/\1,timescaledb'/" /opt/crunchy/conf/postgres/postgresql.conf.template && \
    sed -i '/\\c "\${PG_DATABASE}"$/a CREATE EXTENSION IF NOT EXISTS timescaledb;' /opt/crunchy/bin/postgres/setup.sql && \
    sed -i '/SET application_name="container_setup";$/a CREATE EXTENSION IF NOT EXISTS timescaledb;' /opt/crunchy/bin/postgres/setup.sql
