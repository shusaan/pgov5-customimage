...
RUN if [ "$TARGETPLATFORM" = "linux/amd64" ]; then \
        rpm -i /tmp/pgdg-redhat-repo-latest.noarch_EL-8-x86_64.rpm && microdnf install -y timescaledb_15-2.11.1-1.rhel8 && microdnf clean all; \
    elif [ "$TARGETPLATFORM" = "linux/arm64" ]; then \
        rpm -i /tmp/pgdg-redhat-repo-latest.noarch_EL-8-aarch64.rpm && microdnf install -y timescaledb_15-2.11.1-1.rhel8 && microdnf clean all; \
    fi && rm -rf /tmp/*
USER 26